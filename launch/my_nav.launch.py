...
def launch_setup(context):
    ...
    my_package_path = get_package_share_directory('my_nav_pkg')
    
    navigation_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(my_package_path, 'launch/my_bringup.launch.py')),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'map': os.path.join(slam_package_path, 'maps', map_name + '.yaml'),
            'params_file': os.path.join(get_package_share_directory('my_nav_pkg'), 'config', 'my_nav_params.yaml'),
